...
class WeightMergeUp(nn.Module):
    ...
    def forward(self, up1, up2):
        return self.alpha*up1 + self.beta*up2
...
class HourglassNet_drawing(exkp):
    def __init__(self, heads, num_stacks=2):
        n = 5
        dims = [256, 256, 384, 384, 384, 512]
        modules = [2, 2, 2, 2, 2, 4]

        super(HourglassNet_drawing, self).__init__(
...
def get_large_hourglass_drawing_net(num_layers, heads, head_conv):
    model = HourglassNet_drawing(heads, 2)
    return model
